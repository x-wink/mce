...
export class Fonts {
  ...
  /**
   * 缺字体通知。`get(family)` 查到某 family 尚未加载（只能退回兜底字体）时触发，携带该 family。
   * 消费方（如接了字体库的编辑器）据此**按需异步加载**该字体——覆盖任何字体「出现」场景：
   * 协同远端改字、粘贴、加载文档等，无需上层轮询/扫描文档。加载完成后 'load' 事件会自动触发重排。
   * 去重：同一 family 未加载期间只触发一次（`_requestedMissing`），避免测量热路径重复触发。
   */
  protected _missingListeners = new Set<(family: string) => void>()
  protected _requestedMissing = new Set<string>()

  on(event: 'load', listener: (font: FontLoadedResult) => void): this
  on(event: 'missing', listener: (family: string) => void): this
  on(event: 'load' | 'missing', listener: any): this {
    if (event === 'load')
      this._loadListeners.add(listener)
    else if (event === 'missing')
      this._missingListeners.add(listener)
    return this
  }

  off(event: 'load', listener: (font: FontLoadedResult) => void): this
  off(event: 'missing', listener: (family: string) => void): this
  off(event: 'load' | 'missing', listener: any): this {
    if (event === 'load')
      this._loadListeners.delete(listener)
    else if (event === 'missing')
      this._missingListeners.delete(listener)
    return this
  }
  ...
  /** 查到未加载的 family 时请求加载（去重；无监听者不记录，等有监听者再触发）。 */
  protected _requestMissing(family: string): void {
    if (!family || this._missingListeners.size === 0 || this._requestedMissing.has(family))
      return
    this._requestedMissing.add(family)
    this._missingListeners.forEach(listener => listener(family))
  }
  ...
  get(familyOrUrl?: string): FontLoadedResult | undefined {
    ...
    if (familyOrUrl) {
      ...
      // 该 family 尚未加载（只能退兜底）→ 通知消费方按需加载
      if (!font)
        this._requestMissing(familyOrUrl)
    }
  ...
  set(family: string, font: FontLoadedResult): this {
    ...
    this._requestedMissing.delete(family) // 已加载：清掉「已请求」标记（若日后被 delete 可再次请求）
    return this
  }
